...
def on_msg(*params):
    ...
    if params[0].message.textMessage.text == 'привет' or params[0].message.textMessage.text == '/start':
        bot.messaging.send_message(params[0].peer,
                                   ...
                                   "для получения  отчета напишите: '/stats' (Внимание человек ,отправляющий запросы на отправку опросов и только он может получить данные по запросу , причем подразумеевается,что у одного бота один админ , как и написано в таске" + "\n"
                                   + "(!!из соображений логики, cчитается, что голос одного человека учитывается не более 1 раза за каждый из вариантов!!)" + "\n" +
                                   "('/stats' возвращает общую статистику по всем опросам(в силу невозможности их дробить из-за недостатков апи) а именно , cколько людей проголосовали за какой вариант  "+"\n" +"(!!!поэтому  не начинайте следующий опрос пока не подвели статистику по предыдущему!!!)"+"\n" +"!ВАЖНО!после /stats  удаляются все данные")
