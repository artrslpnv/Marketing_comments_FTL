...
def on_msg(*params):
    ...
    if params[0].message.textMessage.text == 'привет' or params[0].message.textMessage.text == '/start':
        bot.messaging.send_message(params[0].peer,
                                   ...
                                   + "/create;Название опроса;Первый Вариант;Второй Вариант;и далее варианты через ';' после последнего варианта не ставить!" + "\n" +
                                   "Для получения  отчета напишите: '/stats' (Внимание человек ,отправляющий запросы на отправку опросов и только он может получить данные по запросу , причем подразумеевается,что у одного бота один админ." + "\n"
                                   + "(!!из соображений логики, cчитается, что голос одного человека учитывается не более 1 раза за каждый из вариантов!!)" + "\n" +
                                   "('/stats' возвращает общую статистику по всем опросам(в силу того , что у опроса нет параметра айди) а именно , cколько людей проголосовали за какой вариант  "+"\n" +"(!!!Поэтому  не начинайте следующий опрос пока не подвели статистику по предыдущему!!!)"+"\n" +"!ВАЖНО!после /stats  удаляются все данные")
        ...
        for i in range(2, len(variants)):
            ...
            if not variants[i] in was_value_clicked_by:
        ...
        if params[0].peer == admin:
            ...
            bot.messaging.send_message(params[0].peer,"Нельзя просмотреть статистику , так как вы не отправляли опросов или вы не админ")
    else:
